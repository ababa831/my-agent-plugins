...
3. **認証情報・シークレットの排除**:
   - シークレット、API キー、トークン、個人情報をプラグイン内（`mcp.json` の `env` や `headers` を含む）にコミットしてはなりません。仕様上も禁止されています。
4. **バージョン管理の同期**:
...
- [ ] 既存の公開済みプラグインに変更を加えた場合、すべてのマニフェスト（`plugin.json`, `.cursor-plugin/`, `.codex-plugin/`, `.claude-plugin/`）の `version` を更新する。
- [ ] プラグインカタログに変更がある場合、3つのマーケットプレイスマニフェスト（`.cursor-plugin/`, `.agents/plugins/`, `.claude-plugin/`）を同期する。
