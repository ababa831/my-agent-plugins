...
└── plugins/
    ...
    │   ├── claude/hooks.json              # Claude Code 固有の hook（SessionStart / PostToolUse）
    │   ├── claude/check_banned_words.py   # PostToolUse hook が呼ぶ「正本」検出スクリプト
    │   └── skills/
...
| `japanese-writing` | `japanese-tech-writing`（技術文書の文章規範）、`cognitive-rhythm-writing`（認知リズム設計）、`semantic-generation`（対応表先行生成）。常時適用の共通ルール（返信は日本語で書く、「正本」という語を使わない）も同梱し、Cursor では rule、Claude Code では SessionStart hook で読み込む。Claude Code では「正本」を書き込んだときに書き直しを促す PostToolUse hook も持つ |
...
- 「正本」禁止の機械的なチェックは、`japanese-writing` の Claude Code 用 PostToolUse hook だけとする。CI・lint・他クライアント向けの仕組みは、ルールと hook では防げない失敗が繰り返し観測されるまで追加しない。
