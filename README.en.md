...
| Plugin | Contents |
| :-- | :-- |
| `development-rules` | `git-development-rules` (minimal Git development rules: Conventional Commits, TDD, pull request workflow, and related practices) and `evidence-driven-engineering` (back diagnoses with code and observations, verify running behavior, make verification repeatable, prevent recurring corrections with types, lint, CI, and similar mechanisms, and guidance for delegation and skill evaluation). In Claude Code, `git-development-rules` is loaded into every session via a SessionStart hook |
